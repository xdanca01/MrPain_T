...
**/
	...
	tBTNodePtr kam = *RootPtr, predchozi = kam;
    while(kam != NULL)
    {
        predchozi = kam;
        if(kam->Cont == Content) return;
        if(kam->Cont > Content) kam = kam->LPtr;
        else kam = kam->RPtr;
    }
    kam = malloc(sizeof(struct tBTNode));
    kam->Cont = Content;
    kam->LPtr = NULL;
    kam->RPtr = NULL;
    if(kam == NULL) return;
    if(predchozi != NULL)
    {
        if(predchozi->Cont > Content) predchozi->LPtr = kam;
        else predchozi->RPtr = kam;
    }
    else *RootPtr = kam;
}
...
**/
    ...
    Leftmost_Preorder(actual, zasobnik);
    while(SEmptyP(zasobnik) == false)
    {
        actual = (STopPopP(zasobnik))->RPtr;
        if(actual != NULL)
        {
            BTWorkOut(actual);
            SPushP(zasobnik, actual);
            if(actual->LPtr != NULL) Leftmost_Preorder(actual->LPtr, zasobnik);
        }
    }
    free(zasobnik);
    return;
...
**/
    ...
    while(SEmptyP(zasobnik) == false)
    {   
        tBTNodePtr actual = STopPopP(zasobnik);
        BTWorkOut(actual);
        if(actual->RPtr != NULL)
        {
            Leftmost_Inorder(actual->RPtr, zasobnik);
        }
    }   
    free(zasobnik);
    return;
...
**/
    ...
    tStackP *zasobnikP = malloc(sizeof(tStackP));
    if(zasobnikP == NULL) return;
    SInitP(zasobnikP);
    tStackB *zasobnikB = malloc(sizeof(tStackB));
    if(zasobnikB == NULL)
    {
        free(zasobnikP);
        return;
    ...
    tBTNodePtr actual, pravy;
    bool podminka;
    while(SEmptyP(zasobnikP) == false && SEmptyB(zasobnikB) == false)
    {
        actual = STopPopP(zasobnikP);
        pravy = actual->RPtr;
        podminka = STopPopB(zasobnikB);
        if(pravy != NULL && podminka != false)
        {
            SPushP(zasobnikP, actual);
            SPushB(zasobnikB, false);
            Leftmost_Postorder(pravy, zasobnikP, zasobnikB);
        }
        else BTWorkOut(actual);
    }
    free(zasobnikP);
    free(zasobnikB);
...
**/
    ...
    if(zasobnik == NULL) return;
    SInitP(zasobnik);
    tBTNodePtr zaloha = *RootPtr, zrusit;
    while(zaloha != NULL)
    {
        if(zaloha->LPtr != NULL)
        {
            SPushP(zasobnik, zaloha);
            zaloha = zaloha->LPtr;
        }
        else if(zaloha->RPtr != NULL)
        {
            SPushP(zasobnik, zaloha);
            zaloha = zaloha->RPtr;
        }
        else if(SEmptyP(zasobnik) == false)
        {
            zrusit = zaloha;
            zaloha = STopPopP(zasobnik);
            if(zaloha->LPtr == zrusit)
            {
                free(zrusit);
                zaloha->LPtr = NULL;
            }
