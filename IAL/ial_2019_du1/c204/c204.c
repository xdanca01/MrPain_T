...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	//pokud je prazdny vratime 0
	//dokud neni prazdny ++i a zapisujeme hodnoty do pole
	char znak;
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
    ...
    char vrchol;
    if(stackEmpty(s) != 1) stackTop(s,&vrchol);
    if(c == '(') stackPush(s,c);
    else if((c == '*' || c == '/' ) && (stackEmpty(s) == 1 || vrchol == '(' || vrchol == '+' || vrchol == '-')) stackPush(s,c);
	else if((c == '+' || c == '-') && (stackEmpty(s) == 1 || vrchol == '(')) stackPush(s,c);
    while((c == '+' || c == '-') && (vrchol == '*' || vrchol == '/' || vrchol == '+' || vrchol == '-'))
    {
        postExpr[*postLen] = s->arr[s->top];
        stackPop(s);
        ++(*postLen);
        if(stackEmpty(s) != 1) stackTop(s,&vrchol);
        if(stackEmpty(s) == 1 || vrchol == '(')
        {
            stackPush(s,c);
            break;
        }
    }
    while((c == '*' || c == '/') && (vrchol == '/' || vrchol == '*'))
    {
        postExpr[*postLen] = s->arr[s->top];
        stackPop(s);
        ++(*postLen);
...
char* infix2postfix (const char* infExpr) {
	...
	tStack s;
	stackInit(&s);
	//pomocna promenna pro uchovani pozice vystupniho retezce
	unsigned a = 0;
	//char znak;
	char* retezec = (char *)malloc(MAX_LEN * sizeof(char));
	if (retezec == NULL) return NULL;
	//retezec by mel koncit znakem =
	for (unsigned i = 0; infExpr[i] != '\0'; ++i)
	{
        if(infExpr[i] == '*' || infExpr[i] == '/' || infExpr[i] == '+' || infExpr[i] == '-' || infExpr[i] == '=' || infExpr[i] == '(' || infExpr[i] == ')') doOperation(&s,infExpr[i],retezec,&a);
        else
        {
            retezec[a] = infExpr[i];
            ++a;
        ...
        /*
        //pokud neni stack empty beru porovnavaci znak ze stacku
		if(stackEmpty(&s) != 1) stackTop(&s, &znak);
		//pokud je znak * / a zasobnik je empty nebo je tam operator s nizsi prioritou zapis na zasobnik
		if ((infExpr[i] == '*' || infExpr[i] == '/') && (stackEmpty(&s) == 1 || znak == '+' || znak == '-'))
		{
			stackPush(&s, infExpr[i]);
		}
		else if ((infExpr[i] == '*' || infExpr[i] == '/') && (znak == '*' || znak == '/'))
		{
			stackTop(&s, &retezec[a]);
			++a;
			stackPop(&s);
            --i;
		}
		else if ((infExpr[i] == '+' || infExpr[i] == '-') && (stackEmpty(&s) == 1 || znak == '('))
		{
			stackPush(&s, infExpr[i]);
		}
		else if ((infExpr[i] == '+' || infExpr[i] == '-') && (znak == '+' || znak == '-'))
		...
		else if (infExpr[i] == ')') untilLeftPar(&s, retezec, &a);
		else if (infExpr[i] == '=')
		{
			untilLeftPar(&s, retezec, &a);
            retezec[a] = '=';
		}
		else
		{
