...
GLuint TextureUtils::load_texture_cube(const std::filesystem::path filename_px, const std::filesystem::path filename_nx, const std::filesystem::path filename_py,
    ...
    // glTextureStorage3D(texture, 1, GL_RGBA8, 1024, 1024, 6);

    std::vector<std::filesystem::path> files = {filename_px, filename_nx, filename_py, filename_ny, filename_pz, filename_nz};
    for (int i = 0; i < 6; i++) {
        GLsizei width, height;
        unsigned char* loaded_data = load_image(files[i], flip_y, width, height);
        // glTextureSubImage3D(texture, 0, 0, 0, i, width, height, 1, GL_RGBA, GL_UNSIGNED_BYTE, loaded_data);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, loaded_data);
