...
void main()
{
	vec3 pos = texture(positions_tex, in_data.tex_coord).rgb;

	if(length(lights[in_data.light_idx].position.xyz - pos) > light_range)
		discard;

	vec3 norm = texture(normals_tex, in_data.tex_coord).rgb;
	vec3 alb = texture(albedo_tex, in_data.tex_coord).rgb;
	...
	vec3 L_not_normalized = lights[in_data.light_idx].position.xyz - pos * lights[in_data.light_idx].position.w;
