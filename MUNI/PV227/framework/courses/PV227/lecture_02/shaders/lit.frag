...
void main()
{
	...
	vec4 Vworld = shadow_matrix * vec4(in_data.position_ws, 1.0f);
	/*Vworld = Vworld/Vworld.w;
	float distanceFromLight = textureProj(shadow_tex, Vworld), result = 1.0f;
	if(distanceFromLight < Vworld.z){
		result = 0.0f;
	}*/
	float result = textureProj(shadow_tex, Vworld);
	...
	vec3 final_light = mat_ambient * amb + result* mat_diffuse * dif + result* material.specular * spe;

	// Outputs the final light color.
	final_color = vec4(final_light, material.alpha);
	//final_color = vec4(Vworld.z);
}