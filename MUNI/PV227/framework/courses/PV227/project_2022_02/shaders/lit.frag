...
void main()
{
	...
	//float shadow_factor = texture(shadow_tex, shadow_tex_coord.xy/ shadow_tex_coord.w).z;
	//float shadow_factor = (shadow_tex_coord.z) <= textureLod(shadow_tex, shadow_tex_coord.xy, 0).r ? 1.0 : 0.0;
	float shadow_factor = texture(shadow_tex, shadow_tex_coord.xy).r >= (shadow_tex_coord.z) - 0.005 ? 1.0 : 0.0;
	...
	if(shadow_factor >= 0.1 && use_snow && dot(N, vec3(0.0f, 1.0f, 0.0f)) >= 0.1 ){
		float accumulated_snow = texture(snow_tex, shadow_tex_coord.xy).r;
		mat_diffuse = mix(mat_diffuse, vec3(1.0), accumulated_snow) * dot(N, vec3(0.0f, 1.0f, 0.0f));
	}
	...
	//final_color = vec4(texture(snow_tex, shadow_tex_coord.xy).rgb, 1.0);
	//final_color = vec4(N, 1.0);
	//final_color = vec4(vec3(shadow_factor), 1.0);
	//final_color = vec4(texture(shadow_tex, shadow_tex_coord.xy / shadow_tex_coord.w).rgb, 1.0);
}